...
s1.add('b');
s1.add('o');
s1.add('h'); //не добавит так как есть уже такой элемент
...
const s3 = new Set(['one', 'two', 'four']); // обратите внимание, как просто можно добавить массив в набор!
...
const f4 = () => {
    ...
    document.querySelector('.out-4').innerText = s4.has(i4);
}
...
const f5 = () => {
    document.querySelector('.out-5').innerText = s5.size;
}
...
const a6 = [1, 2, 3, 4, 5, 3, 4, 5, 2, 4, 5, 3, 24, 5, 2, 4, 56, 4, 3, 2, 335, 2, 23, 41, 3, 4, 1, 1, 4, 2, 2, 4, 5, 24, 5, 3, 22, 56];

const f6 = () => {
    const a6Set = new Set(a6);

    document.querySelector('.out-6').innerText = a6Set.size;
    console.log(a6Set);
}
...
// При нажатии b-8 выполняете функцию f8. Функция должна перебрать набор s8 и добавить в массив ar8 только числа больше 5. Выведите массив в консоль.
 ...
 const s8 = new Set([1, 2, 3, 4, 5, 3, 4, 7, 9, 5, 7, 8, 9, 23, 45, 5, 2, 4, 5, 3, 24, 5, 2, 4, 56, 4, 3, 2, 335, 2, 23, 41, 3, 4, 1, 1, 4, 2, 2, 4, 5, 24, 5, 3, 22, 56]);
let ar8 = [];
// console.log(s8); //здесь у нас уже все уникальтные элементы
const f8 = () => {
    for (let item of s8){
        if (item > 5){
            ar8+= `${item} `;
...
//  преобразовывать его в строку, причем после каждого символа строки должен быть пробел. Функция должна возвращать результирующую строку.
