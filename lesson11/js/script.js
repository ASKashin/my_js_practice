...
const f1 = () => {
    // в задании дописал только эти 2 строки ниже
    const i1 = document.querySelector('.i-1').value;
    d1.push(i1);

    showArr('.out-1', d1);  //здесь мы вызываем функцию которая выводит массив и в качестве пароаметра передаем куда выводить и наш массив
    // не могу понять как мы так без получения отдельнор берем out-1  или все дело в названии переменной domElem ( заменят походу запись document.querySelector('.out-1')- погуглил ниче не понял
}
...
const f2 = () => {
    ...
    d1.pop(); // удаляет последний элемент
...
const f3 = () => {
    ...
    d1.shift(); // удаляет первый элемен
...
const f4 = () => {
    ...
    d1.push(i4); // добавляет в конец строки
// не понял прикола этого задания оно такое же один в один как первое
    showArr('.out-4', d1);
}
...
const f5 = () => {
    ...
    d1.unshift(i5); //добавляет вначало строки
...
const f6 = () => {
    ...
    const d6Length = d6.length //здесть если мы возьмем длинну массива то она всегда будет = index последнего + 1
    d6[d6Length] = i6; // здсь я присваиваю данному индексу значение из i6
    console.log(d6);
    // ох я тут и поломал мозг час почти ушел на это)))но я победил вроде)
    showArr('.out-6', d6);
}
...
const f7 = () => {
    ...
    const lastIndex = d7.length - 1;
//ваще хер его знает только через splice
//     delete d7[lastIndex];
    d7.splice(lastIndex, 1)
    console.log(d7);
    showArr('.out-7', d7);
}
...
const f8 = () => {
    // тут тоже только через splice я могу сделать больше ну не как не могу придумать
...
const f11 = () => {
    const i11 = Number(document.querySelector('.i-11').value);
    document.querySelector('.out-11').innerText = d11.indexOf(i11);
